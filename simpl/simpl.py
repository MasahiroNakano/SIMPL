...
class SIMPL:
    ...
    def __init__(
        ...
        self.odd_minute_mask = jnp.stack(
            [jnp.array(self.time // 60 % 2 == 0)] * self.N_neurons, axis=1
        ...
        sigma0 = (1 / self.T) * (
            ((self.Xb - mu0).T) @ (self.Xb - mu0)
        )  # initial state covariance (estimate from behaviour)
        speed_sigma = speed_prior * self.dt
        behaviour_sigma = (
            behaviour_prior if behaviour_prior is not None else 1e6
        )  # if no behaviour prior, set to a large value (effectively no prior)
        lam = behaviour_sigma**2 / (speed_sigma**2 + behaviour_sigma**2)
        sigma_eff_square = (
            speed_sigma**2 * behaviour_sigma**2 / (speed_sigma**2 + behaviour_sigma**2)
        )
        ...
        self.R_base = observation_noise_std**2 * jnp.eye(
            self.D
        ...
        self.dim = (
            self.environment.dim
        )  # as ordered in positon variables X = ['x', 'y', ...]
        ...
        self.results = xr.merge(
            [
                self.results,
                self.dict_to_dataset(
                    {"Xb": self.Xb, "Y": self.Y, "spike_mask": self.spike_mask}
                ),
            ]
        )  # add spikes and behaviour to the results
        self.loglikelihoods = xr.Dataset(
            coords={"epoch": jnp.array([], dtype=int)}
        )  # a smaller dict just to save likelihoods for online evaluation during training
        ...
        if "Xt" in list(self.data.keys()):
            ...
            self.results = xr.merge(
                [self.results, self.dict_to_dataset({"Xt": self.Xt})]
            )
        if "Ft" in list(
            self.data.keys()
        ):  # interpolate the "true" receptive fields onto the coordinates of the environment
            Ft = (
                self.data.Ft.interp(
                    **self.environment.coords_dict,
                    method="linear",
            ...
            Ft = Ft.transpose(
                "neuron", *self.environment.dim
            )  # make coord order matches those of this class
            self.Ft = jnp.array(Ft.values).reshape(
                self.N_neurons, self.N_bins
            )  # flatten to shape (N_neurons, N_bins)
            self.Ft = jnp.where(
                self.Ft < 0, 0, self.Ft
            )  # threshold Ft at 0 just in case they weren't already
            self.results = xr.merge(
                [self.results, self.dict_to_dataset({"Ft": self.Ft})]
            )
    ...
    def train_N_epochs(self, N: int = 5, verbose: bool = True):
        ...
        pbar = (
            tqdm(range(self.epoch, self.epoch + N))
            if verbose
            else range(self.epoch, self.epoch + N)
        )
        ...
        for epoch in pbar:
            try:
                ...
                break
        if self.evaluate_each_epoch == False:
            self.evaluate_epoch()  # Always evaluate at the end of training if not done each epoch
    ...
    def train_epoch(
        ...
        loglikelihoods = self.dict_to_dataset(
            self.get_loglikelihoods(Y=self.Y, FX=self.M["FX"])
        ).expand_dims({"epoch": [self.epoch]})
        self.loglikelihoods = xr.concat(
            [self.loglikelihoods, loglikelihoods], dim="epoch", data_vars="minimal"
        )
    ...
    def evaluate_epoch(self):
        ...
        results = self.dict_to_dataset({**self.M, **self.E, **evals}).expand_dims(
            {"epoch": [self.epoch]}
        )
        self.results = xr.concat(
            [self.results, results], dim="epoch", data_vars="minimal"
        )
        return
    ...
    def _E_step(self, Y: jnp.ndarray, F: jnp.array) -> xr.Dataset:
        ...
        observation_noise_test = (
            jnp.where(no_spikes[:, None, None], jnp.eye(self.D) * 1e6, sigma_l_test)
            + self.R_base
        )
    ...
    def _M_step(self, Y: jnp.ndarray, X: jnp.ndarray) -> xr.Dataset:
        ...
        stacked_masks = jnp.array(
            [self.spike_mask, self.odd_minute_mask, self.even_minute_mask]
        )
    ...
    def get_loglikelihoods(self, Y: jnp.ndarray, FX: jnp.ndarray) -> dict:
        ...
        LLs = {}
        logPYXF = (
            poisson_log_likelihood_trajectory(Y, FX, mask=self.spike_mask).sum()
            / self.spike_mask.sum()
        )
        logPYXF_test = (
            poisson_log_likelihood_trajectory(Y, FX, mask=~self.spike_mask).sum()
    ...
    def get_metrics(
        ...
        # SPATIAL INFORMATION
        if F is not None:
            F_pdf = (F + 1e-6) / jnp.sum(F, axis=1)[
                :, None
            ]  # normalise the place fields
            I_F = jnp.sum(
                F_pdf * jnp.log(F_pdf), axis=1
            )  # spatial information of the place fields
            metrics["information"] = I_F

        # SPARSITY
        if F is not None:
            rho_F = jnp.mean(
                F < 1.0 * self.dt, axis=1
            )  # fraction of bins where the firing is greater less than 1 Hz
    ...
    def calculate_baselines(self):
        ...
        if self.ground_truth_available == False:
            warnings.warn(
                "Ground truth data not available, so the baselines cannot be calculated."
            )
            return
        ...
        if self.Ft is not None:
            ...
            results = self.dict_to_dataset({**M, **E, **evals}).expand_dims(
                {"epoch": [-2]}
            )
            self.results = xr.concat(
                [self.results, results], dim="epoch", data_vars="minimal"
            ...
            warnings.warn(
                "Exact place fields not provided so baselines against the exact model cannot be calculated."
            )
    ...
    def interpolate_firing_rates(self, X, F):
        ...
        """
        F = np.array(F)
        X = np.array(X)  # only need this for jax>0.4.28 ?!?!
        data = self.dict_to_dataset(
            {"F": F, "X": X}
        )  # reshape F into the correct shape
        coord_args = {
            dim: data.X.sel(dim=dim) for dim in self.dim
        }  # get the coordinates of the latent positions
        FX = data.F.sel(
            **coord_args, method="nearest"
        ).T  # interpolate the fields onto the latent positions using xarray.sel()
        return FX.data
    ...
    def dict_to_dataset(self, data: dict, coords: dict = None):
        ...
        for variable_name in data.keys():
            if variable_name in self.variable_info_dict:
                ...
                intended_variable_shape = tuple(
                    [len(variable_coords[c]) for c in variable_info["dims"]]
                )  # shape implied by the dimensions
                if "reshape" in variable_info and variable_info["reshape"]:
                    variable_data = data[variable_name].reshape(
                        intended_variable_shape
                ...
                warnings.warn(
                    f"Variable {variable_name} not recognised, it will be saved without coordinate or dimension info unless you add it to the variable_info_dict in the init_variables_dict method."
                )
    ...
    def analyse_place_fields(self, F: jnp.array):
        ...
        env_size = (
            self.environment.flattened_discretised_coords.shape[0] * volume_element
        )
        ...
        for n in range(self.N_neurons):
            ...
            for f in range(1, min(self.N_PFmax, putative_pfs_count + 1)):
                ...
                perimeter_dilated = dx * skimage.measure.perimeter(
                    scipy.ndimage.binary_dilation(pf_mask)
                )
    ...
    def _set_pbar_desc(self, pbar):
        ...
        try:
            ...
            likelihood_test = self.loglikelihoods.logPYXF_test.sel(
                epoch=self.epoch
            ).values
            ...
            pbar.set_description(
                f"Epoch {self.epoch}: Train LL: {likelihood:.3f} (Δ{likelihood-likelihood0:.3f}), Test LL: {likelihood_test:.3f} (Δ{likelihood_test-likelihood_test0:.3f})"
            )
        except:
            try:
